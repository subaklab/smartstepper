...
public class BleWrapper {
    ...
    private static final String TAG = "BLETEST BleWrapper" ;
    ...
    public void getCharacteristicValue(BluetoothGattCharacteristic ch) {
        ...
        if(uuid.equals(BleDefinedUUIDs.Characteristic.LBS)){
            Log.d(TAG, "get LBS");
            int format =  BluetoothGattCharacteristic.FORMAT_UINT8;
            intValue = ch.getIntValue(format,0);
            strValue = String.valueOf(intValue);
            Log.d(TAG, "LBS =" +  strValue );
        }

        else if (uuid.equals(BleDefinedUUIDs.Characteristic.LBS_BUTTON)) {
            Log.d(TAG, "get LBS Button ");
            int format = BluetoothGattCharacteristic.FORMAT_UINT8;
            intValue = ch.getIntValue(format, 0);
            strValue  = String.valueOf(intValue);
            Log.d(TAG, "LBS Button =" +  strValue );
        }

        else if (uuid.equals(BleDefinedUUIDs.Characteristic.LBS_LED)) {
            Log.d(TAG, "get LBS LED ");
//            int format = BluetoothGattCharacteristic.FORMAT_UINT8;
//            byte [] dataBytes = ch.getValue();
//            if (dataBytes.length > 0){
//            String rxData = String.valueOf(dataBytes[0]);
//            Log.d(TAG, "LBS LED =" + rxData);
//            }
            int format = BluetoothGattCharacteristic.FORMAT_UINT8;
            intValue = ch.getIntValue(format, 0);
            strValue = String.valueOf(intValue);
    ...
    private final BluetoothGattCallback mBleCallback = new BluetoothGattCallback() {
                                         ...
                                         int status)
        {
            ...
            Log.d (TAG, "try read " + characteristic.toString() );
            if (status == BluetoothGatt.GATT_SUCCESS) {
